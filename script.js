...
function verificarRespostas(){
    ...
    for(let i = 0; i < numCerto; i++){
        ...
        let respostaCorreta1 =  document.getElementById(`respostaCorreta${i + 1}`).value;
        ...
        let imagemIncorreta1 = document.getElementById(`imagemIncorreta1${i + 1}`).value;
        let respostaIncorreta2 = document.getElementById(`respostaIncorreta2${i + 1}`).value;
        let imagemIncorreta2 = document.getElementById(`imagemIncorreta2${i + 1}`).value;
        let respostaIncorreta3 = document.getElementById(`respostaIncorreta3${i + 1}`).value;
        let imagemIncorreta3 = document.getElementById(`imagemIncorreta3${i + 1}`).value;

        

        if (titulo1.length >= 20){
            if(corFundo1.includes("#") && corFundo1.length === 7){
                if(respostaCorreta1 !== "" && respostaIncorreta1 !== "" && imagemCorreta.includes("https://") && imagemIncorreta1.includes("https://")){
                    objetoRespostas = {
                        title: titulo1,
                        color: corFundo1,
                        answers: [
                ...
                },
                {
                    ...
                    if(respostaIncorreta2 !== "" && imagemIncorreta2.includes("https://")){
                        objetoRespostas.answers.push({
                            text: respostaIncorreta2,
                            image: imagemIncorreta2,
                            isCorrectAnswer: false
                        });
                        if (respostaIncorreta3 !== "" && imagemIncorreta3.includes("https://")){
                            objetoRespostas.answers.push({
    ...
    if(objetoPrincipal.questions.length < numCerto || objetoPrincipal.questions.length > numCerto){
        objetoPrincipal.questions.length = 0;
    }
...
function verificarRespostasNiveis(){
    for(let i = 0; i < numNiveis; i++){
        ...
        let porcentagemNivel = document.getElementById(`porcentagemNivel${i + 1}`).value;
        ...
        if (tituloNivel.length >= 10 && Number(porcentagemNivel) >= 0 && 100 >= Number(porcentagemNivel)  && imagemNivel.includes("https://") && descricaoNivel.length >= 30) {
